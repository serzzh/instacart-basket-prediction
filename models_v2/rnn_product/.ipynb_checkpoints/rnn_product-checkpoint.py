...
from tf_utils import lstm_layer, time_distributed_dense_layer, DenseLayer, sequence_log_loss, temporal_convolution_layer
...
class rnn(TFBaseModel):
    ...
    def get_input_sequences(self):
        ...
        self.user_id = tf.keras.Input(shape=(), dtype=tf.dtypes.int32)
        self.product_id = tf.keras.Input(shape=(), dtype=tf.dtypes.int32)
        self.aisle_id = tf.keras.Input(shape=(), dtype=tf.dtypes.int32)
        self.department_id = tf.keras.Input(shape=(), dtype=tf.dtypes.int32)
        self.is_none = tf.keras.Input(shape=(), dtype=tf.dtypes.int32)
        self.history_length = tf.keras.Input(shape=(), dtype=tf.dtypes.int32)

        self.is_ordered_history = tf.keras.Input(shape=(None, 100), dtype=tf.dtypes.int32)
        self.index_in_order_history = tf.keras.Input(shape=(None, 100), dtype=tf.dtypes.int32)
        self.order_dow_history = tf.keras.Input(shape=(None, 100), dtype=tf.dtypes.int32)
        self.order_hour_history = tf.keras.Input(shape=(None, 100), dtype=tf.dtypes.int32)
        self.days_since_prior_order_history = tf.keras.Input(shape=(None, 100), dtype=tf.dtypes.int32)
        self.order_size_history = tf.keras.Input(shape=(None, 100), dtype=tf.dtypes.int32)
        self.reorder_size_history = tf.keras.Input(shape=(None, 100), dtype=tf.dtypes.int32)
        self.order_is_weekend_history = tf.keras.Input(shape=(None, 100), dtype=tf.dtypes.int32)
        self.order_part_of_day_history = tf.keras.Input(shape=(None, 100), dtype=tf.dtypes.int32)
        self.order_number_history = tf.keras.Input(shape=(None, 100), dtype=tf.dtypes.int32)
        self.product_name = tf.keras.Input(shape=(None, 30), dtype=tf.dtypes.int32)
        self.product_name_length = tf.keras.Input(shape=(), dtype=tf.dtypes.int32)
        self.next_is_ordered = tf.keras.Input(shape=(None, 100), dtype=tf.dtypes.int32)

        self.keep_prob = tf.keras.Input(shape=(), dtype=tf.dtypes.float32)
        self.is_training = tf.keras.Input(shape=(), dtype=tf.dtypes.bool)

        # product data
        product_embeddings = tf.Variable(
            tf.zeros(shape=(50000, self.lstm_size)),
            name='product_embeddings',
            dtype=tf.float32
        )
        aisle_embeddings = tf.Variable(
            tf.zeros(shape=(50, 50)),
            name='aisle_embeddings',
            dtype=tf.float32
        )
        department_embeddings = tf.Variable(
            tf.zeros(shape=(50, 0)),
        ...
        dense = tf.keras.layers.Dense(100, input_shape=(product_names.shape[0],), activation=tf.nn.relu)
        product_names = dense(product_names)
        ...
        user_embeddings = tf.Variable(
            tf.zeros(shape=(207000, self.lstm_size)),
