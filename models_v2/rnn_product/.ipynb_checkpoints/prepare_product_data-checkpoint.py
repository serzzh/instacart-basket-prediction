...
if __name__ == '__main__':
    ...
    product_data = product_data.loc[:int(len(product_data)/100),:]
    ...
    for i, row in product_data.iterrows():
        ...
        is_ordered_history[i, :], history_length[i] = pad_1d(pool.map(int, row['is_ordered_history'].split()), 100)
        index_in_order_history[i, :], _ = pad_1d(pool.map(int, row['index_in_order_history'].split()), 100)
        order_dow_history[i, :], _ = pad_1d(pool.map(int, row['order_dow_history'].split()), 100)
        order_hour_history[i, :], _ = pad_1d(pool.map(int, row['order_hour_history'].split()), 100)
        days_since_prior_order_history[i, :], _ = pad_1d(pool.map(int, row['days_since_prior_order_history'].split()), 100)
        order_size_history[i, :], _ = pad_1d(pool.map(int, row['order_size_history'].split()), 100)
        reorder_size_history[i, :], _ = pad_1d(pool.map(int, row['reorder_size_history'].split()), 100)
        order_is_weekend_history[i, :], _ = pad_1d(pool.map(int, row['order_is_weekend_history'].split()), 100)
        order_part_of_day_history[i, :], _ = pad_1d(pool.map(int, row['order_part_of_day_history'].split()), 100)
        order_number_history[i, :], _ = pad_1d(pool.map(int, row['order_number_history'].split()), 100)
        product_name[i, :], product_name_length[i] = pad_1d(pool.map(int, row['product_name_encoded'].split()), 30)
