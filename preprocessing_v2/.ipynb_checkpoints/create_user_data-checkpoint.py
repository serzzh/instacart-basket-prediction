...
if __name__ == '__main__':
    ...
    df = orders.iloc.merge(order_products, how='left', on='order_id')
    ...
    user_data = applyParallel(df.groupby('user_id', sort=False), parse_user)[0].reset_index().rename(columns={'index':'user_id'})
    user_data['user_id'] += 1
    
    #user_data = df.groupby('user_id', sort=False).apply(parse_user).reset_index()
    user_data.to_csv('../data/processed/user_data_old.csv', index=False)
